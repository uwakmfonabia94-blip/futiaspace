...
export function renderSignup() {
  ...
  main.innerHTML = `
    <div class="auth-container" id="signupContainer">
      ...
      <p class="auth-subtitle" id="stepDescription">Step 1 of ${totalSteps}: Your account details</p>
      ...
      <form id="signupForm" style="flex:1;">
        <div id="step1" class="step-content">
          ...
          <div class="form-group">
            ...
            <button type="button" class="password-toggle" id="togglePassword"><i data-lucide="eye"></i></button>
          </div>
          <div class="form-group">
        ...
        <div id="step2" class="step-content" style="display:none;">
          ...
          <div class="form-group">
            ...
            <select id="level" required><option value="">Select level</option><option value="100">100</option><option value="200">200</option><option value="300">300</option></select>
          </div>
          <div class="form-group">
        ...
        <div id="step3" class="step-content" style="display:none;">
          <div class="form-group">
            <label for="bio">Bio (optional but recommended)</label>
            <textarea id="bio" rows="3" placeholder="Tell us about yourself…"></textarea>
          </div>
        </div>
        <div id="step4" class="step-content" style="display:none;">
          <p style="color:var(--color-text-secondary); font-size:14px; margin-bottom:12px;">Add a profile photo – it helps people recognise you on campus.</p>
          <div class="avatar-upload" id="avatarContainer">
            ...
            <small style="color:var(--text-muted);">Required. Helps classmates recognise you.</small>
          </div>
        </div>
  ...
  function showStep(step) {
    ...
    const labels = ['Your account details', 'Campus info', 'About you', 'Profile photo'];
  ...
  function updateSubmitBtnState() {
    if (currentStep === totalSteps) submitBtn.disabled = !selectedFile;
  }
  ...
  function validateStep(step) {
    ...
    } else if (step === 3) return true;
    else if (step === 4) return !!selectedFile;
    return true;
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      email, password,
      options: { data: { full_name: fullName, department, level, gender, bio: bio || null } }
    });
    ...
    if (selectedFile) {
      try {
        ...
        const { error: uploadError } = await supabase.storage.from('avatars').upload(path, compressedBlob, { upsert: true });
        if (!uploadError) {
...
async function loadDepartments() {
  ...
  if (error) return;
  data.forEach(d => { const option = document.createElement('option'); option.value = d.department; option.textContent = d.department; select.appendChild(option); });
}